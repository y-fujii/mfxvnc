...
impl Comparator for BlockComparator {
	fn compare<F: FnMut( usize, usize, usize, usize )>( prev: &mut [u32], next: &[u32], stride: usize, w: usize, h: usize, mut callback: F ) {
...
impl QuadtreeComparator {
	fn compare_rec<F: FnMut( usize, usize, usize, usize )>( prev: &mut [u32], next: &[u32], stride: usize, x0: usize, y0: usize, x1: usize, y1: usize, callback: &mut F ) -> Option<(usize, usize, usize, usize)> {
		let w = x1 - x0;
		let h = y1 - y0;
		if w * h <= Self::TPIXELS {
	...
	fn compare_block( prev: &mut [u32], next: &[u32], stride: usize, x0: usize, y0: usize, x1: usize, y1: usize ) -> Option<(usize, usize, usize, usize)> {
		let prev = prev.as_mut_ptr();
		let next = next.as_ptr();
		/*
